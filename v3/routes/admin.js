...
// GET /api/admin/scoring-config — returns all scoring config rows
...
// POST /api/admin/scoring-config — update scoring config values
router.post('/scoring-config', (req, res) => {
    try {
        ...
        const update = db.prepare('UPDATE scoring_config SET value = ? WHERE key = ?');
        const updateAll = db.transaction(() => {
            configs.forEach(c => {
                if (c.key && c.value !== undefined) {
                    update.run(parseFloat(c.value), c.key);
                }
            });
        });
        updateAll();

        logAdminAction(req.user.username, 'update_scoring_config', null, null, { configs });
...
// POST /api/admin/scoring-config/reset — reset to defaults
router.post('/scoring-config/reset', (req, res) => {
    try {
        ...
        logAdminAction(req.user.username, 'reset_scoring_config', null, null, {});
        res.json({ success: true, message: 'Scoring config reset to defaults.' });
    } catch (err) {
        console.error('Scoring config reset error:', err);
